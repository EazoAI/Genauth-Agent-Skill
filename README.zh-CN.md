...
面向人类用户和 AI Agent 的 Agent Identity 操作手册。这组 Skills 必须与
`genauth-agent` CLI 配套使用，覆盖用户池登录、公司级
Agent 创建与审批、Agent 设置、Credential、用户授权、Token 以及通过 GenAuth
调用固定 Provider 的完整闭环。

> Skills 是决策、编排和安全规则层；CLI 是唯一执行层。只安装 Skills、没有
> `genauth-agent` CLI，无法完成任何真实操作。
...
Agent Identity Skills 将自然语言意图转换为受约束的 `genauth-agent` CLI
...
`npx skills add` 会为支持的 AI Agent 安装本仓库中的全部 Skills。不要只复制某个
入口 Skill；共享 Skill、领域 Skill 和 references 都是完整执行所必需的。
...
genauth-agent --profile <authorization-profile> --timeout 10m \
  authorizations wait \
...
- 所有请求只通过 `genauth-agent` CLI 进入 GenAuth。
...
Pipeline 中，将相同 commit/Tag 推送到已确认的 GitHub 仓库：

<https://github.com/EazoAI/Genauth-Agent-Skill>

任务通过进程级 Git HTTP Authorization Header 使用 Token；Token 不会写入
remote URL 或仓库配置，并且永远不 force push。
...
- `genauth-agent` CLI：用户和 AI Agent 实际执行命令的唯一执行层。
